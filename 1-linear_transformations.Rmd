...
1. Doing the transformation of a sum of vectors gives the same result as doing the sum of the respective transformations:

$$
L(u + v) = L(u) + L(v)
$$
2. Scaling a vector by a scalar $\lambda$ then doing the transformation gives the same result as doing the transformation of the vector first then scaling the result by $\lambda$:
...
A matrix A is full rank (= all column vectors are linearly independent) if and only if the null space is the zero vector space. You can find the proof of this theorem in the [Khan Academy](https://en.khanacademy.org/) videos in the prerequisites sections.
...
If A is a square matrix, A is invertible if and only if the null space is the zero vector space. You can find the proof of this theorem in the [third lecture](https://www.youtube.com/watch?v=FX4C-JpTFgY) of the MIT course in linear algebra, by Gilbert Strang (timestamps: 27:56-31:50).
...
2. Gauss-Jordan. It contains a calculator that performs Gauss-Jordan column-wise. The algorithm can be run either stepwise, with the option to fast-forward to the final result. As expected, it gives us two outputs: column echelon form (top) and transformation matrix (bottom). With this tab you can get an intuition of the Fundamental Theorem of Linear Algebra.
...
3. rotates all vectors by $\pi/6$ radians (30 degrees) about the origin (hint: use unit circle).
...
This section aims to teach the fundamentals of R to be able to perform linear algebra operations and visualize them. We will start by teaching the basic data structures, categorized by dimensionality (1D: atomic vectors + lists; 2D: matrices + data.frames) and whether they can contain only one value type (homogeneous: atomic vectors and matrices) or more than one (heterogeneous: lists and data.frames). Moreover, here I showcase an example of the main functions we should cover in the hands-on session:
